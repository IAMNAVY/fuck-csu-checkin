...
一个面向 GitHub Actions 的中南大学智慧学工自动打卡脚本。配置 GitHub Secrets 后，工作流会在每天北京时间 **20:13—22:30** 之间随机选择一个时刻执行一次打卡，并在 Actions 日志中保留执行记录。
...
- 在打卡判定中心 100 米范围内生成随机定位点；首次运行会用三个查询点自动估算中心
...
默认工作流在每天北京时间 20:13 启动一个任务，然后随机等待 0—137 分钟，在 20:13—22:30 之间执行打卡：

```text
北京时间 20:13  ─────── 随机等待 ─────── 22:30
```

GitHub Actions 的 cron 使用 UTC，因此配置文件中使用 `13 12 * * *`。GitHub 的调度可能受到队列影响而延迟；工作流会以任务实际启动时间为基准，尽量将执行限制在当天时间窗内。

如需更换时间窗，请同时调整 `.github/workflows/checkin.yml` 中的启动 cron、随机等待上限和截止时间计算逻辑。

### 判定中心缓存

首次在打卡时段内运行时，脚本会以配置坐标为基准查询三个点，并根据接口返回的 `pcMi` 估算固定判定中心；估算通过残差校验后写入本地 `.checkin-center.json`，之后的随机定位点都以该中心为圆心、限制在 100 米内。GitHub Actions 会通过 Actions cache 保存这个文件，后续任务命中同一坐标、坐标系和打卡类别的缓存时无需重复探测。缓存被 GitHub 清理、修改打卡坐标或切换打卡类别时，会自动重新估算；该文件只包含坐标，不包含密码、token 或 casual。
...
3. 查询当前可用的打卡班次。
4. 将输入坐标转换为 GCJ-02；首次运行时查询三个点的 `pcMi`，估算固定判定中心并缓存。
5. 在判定中心 100 米范围内生成本次定位点。
6. 校验定位点是否在学校允许的范围内。
7. 获取地点名称并提交打卡请求。
...
- 本地 `.checkin-center.json` 只保存判定中心及其坐标元数据，不包含密码、token 或 casual；它已加入 `.gitignore`，GitHub Actions 通过缓存跨运行保存。
